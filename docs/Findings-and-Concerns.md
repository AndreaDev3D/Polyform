...
## F-32. Four ways to import a file wrongly, and all four passed the tests

**Severity: Med** — a 4,825-node `.fig` imported with no pages, filled-in subtractions, no masks, and 24 components emptied out. Reported by a user against their own file; fixed in v0.8.

Each defect has a different shape, and every one of them was invisible to a suite that only ever read our own output.

**1. An enum we never read.** Figma's winding rule is `NONZERO | ODD`. We compared against `'EVENODD'` — *our* name for that rule, a string a `.fig` never contains — so **every even-odd path in every import since the feature shipped came in as nonzero**: a subtraction's hole filled itself in, and the boolean looked like a blob. The value is not a guess and never was: the schema is embedded in the file, and `WindingRule => NONZERO, ODD` prints straight out of it.

The test for this asserted the bug. `fillGeometry: [{ windingRule: 'EVENODD' }]` in a fixture, then `expect(node.windingRule).toBe('EVENODD')` — a round trip through our own vocabulary, green forever, testing nothing. That is [F-28](#f-28-an-importer-can-only-be-checked-against-the-thing-it-imported)'s frozen expectation again, in a different file.

**2. A type inferred from our own output.** The walker dropped a node's children when the node *we made* was not a container, under a comment explaining that a shape with children is a boolean and its children are operands. True for a boolean. Also true, accidentally, for anything else that failed to map to a container — and `SYMBOL` and `INSTANCE` were not in the container list, so each component mapped to a bare path made from its background and then had its entire contents deleted. 26 components, 8 instances, in one file. The condition now reads the **Figma** type, which is the thing that actually decides.

**3. A flag of theirs we did not look for.** `internalOnly` marks Figma's holding canvas — component definitions it has moved aside, deleted nodes, brush assets. Figma does not list it in Pages. We imported it as content: 477 pieces of debris, the largest "page" in the document, next to the four real ones.

**4. Coping instead of building.** Figma pages became one Polyform page with each page after the first *shoved sideways* so they did not overlap. That is a workaround for pages the importer declined to create, and it also meant no import ever landed at the coordinates the file specified. A `.fig` page now becomes a page — `page-add` in the same commit as the nodes, so it is one undo — and the starter page of an untouched document is reused so a fresh project gets exactly the file's pages and nothing else.
