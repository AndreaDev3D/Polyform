...
>
> **Status — Sprints A–D core shipped (2026-08-01):** The full engine surface (4.1 geometry incl. lyon tessellation, 4.2 exact CSG — 2.02x faster than polygon-clipping and default, plus scene/commands/constraints/hit-test/components/layout/serialization) has fuzz-proven Rust twins behind the per-module backend switch (ADR-015); spatial index and booleans run on Rust by default. **4.4 (WebGPU) shipped as a beta** with 6/6 pixel-parity fixtures and the **100k-shapes-at-60fps exit test passing** (ADR-016). Remaining: the effects/blend compositor, the 4.5 text stack, and the 4.3/4.6 engine flip (worker + msgpack boundary) — sprint-level detail in [V0.4-Porting-Plan.md](V0.4-Porting-Plan.md).
