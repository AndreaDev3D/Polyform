...
| 5.4 | **`.fig` import investigation** | **L** | — | **Shipped in v0.7 (experimental).** File → Import .fig…: self-describing schema, shape from the file's own flattened geometry, tree rebuilt from GUIDs + fractional indices, images into content-addressed assets, one undoable entry, and a report of everything approximated or dropped. Placement verified corner-for-corner against Figma's matrix on three real exports — 350 nodes, 0 misplaced (F-28, which is also where the six defects the first version shipped with are recorded). **v0.8** made a Figma page into a Polyform page, read their `ODD` winding rule (every even-odd path had been arriving nonzero, so subtractions filled themselves in), carried masks across, and stopped deleting the contents of components — four defects found against a 4,825-node file and recorded in F-32. Remaining: gradient transform decomposition, auto layout recreated, components as components, and the ~90 s a file that size takes to import — 65 s of it writing images one IPC round-trip at a time, not the commit ([fidelity report](research/Fig-Import-Spike.md), ADR-029). |
