...
>
> **Status — Sprints A + B(part 1) shipped (2026-08-01):** `crates/polyform-core` (Rust→WASM) with geometry, shapes and the spatial index ported and fuzz-proven equivalent to TS; per-module backend switch in `engine/backend.ts` (ADR-015); spatial index runs on Rust by default (2.23x faster bulk load). **Item 4.2 is done**: boolean evaluation defaults to exact bezier CSG in Rust (2.02x faster than polygon-clipping, closes F-03; TS fallback kept). The journal-replay contract fixture for 4.3 is in place. Remaining: path offsetting (4.1), the scene-graph/commands move (4.3), WebGPU (4.4), text (4.5), workers (4.6).
