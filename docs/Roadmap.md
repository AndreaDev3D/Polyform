...
| 5.4 | **`.fig` import investigation** | **L** | — | **Shipped in v0.7 (experimental).** File → Import .fig…: self-describing schema, shape from the file's own flattened geometry, tree rebuilt from GUIDs + fractional indices, images into content-addressed assets, one undoable entry, and a report of everything approximated or dropped. 155/158, 60/63, 135/139 nodes on three real exports. Remaining: gradient transform decomposition, auto layout recreated, components as components ([fidelity report](research/Fig-Import-Spike.md), ADR-029). |
