...
**Decision** *(amended at implementation, distribution flipped on user direction)*. **ISNet (DIS, Apache-2.0) downloads once on explicit consent** — the installer stays slim; the model (fp32, ~171 MB, pinned to rembg's release artifact `isnet-general-use.onnx`, SHA-256 `60920e99…` verified on download and on every session's first read) lives in `userData/models` and the feature is fully offline afterwards. fp32 from the official rembg release rather than a quantized copy, because the floating quint8 repackagings ride in AGPL-adjacent distributions — a self-produced quantized artifact is the approved slimming path later. Inference runs on **onnxruntime-web (MIT)** inside a **Web Worker**: WASM execution provider as the universal baseline, WebGPU EP attempted first; ORT's runtime .mjs/.wasm are handed to the worker as blob: URLs from a main-process read (packaged renderers cannot fetch file:// — ADR-015 lesson). Pre/post-processing (1024² resize, −0.5 normalize, min-max matte, upscale, alpha composite, PNG encode) is our own glue — no AGPL code. Document semantics are non-destructive: the cutout becomes a new SHA-256 asset, the fill swaps hashes in one journal entry, `originalAssetHash` (additive optional field) keeps the pre-cutout asset addressable, and "Restore original" swaps back. BiRefNet_lite fp16 (115 MB, MIT) stays pre-approved as a quality tier **only if** real-document acceptance shows ISNet failing.

**Consequences.** Measured on the harness (`POLYFORM_BG_TEST=1`): matte quality gates pass (subject alpha 255, background 0); single-threaded WASM inference ≈ 12.5 s per image (constant in source size — everything infers at 1024²), always off the UI thread with live progress states. Two documented speed paths, both contained: the WebGPU EP currently falls back (ORT runtime-file flavor mismatch under blob URLs — `webgpuInit` missing; fix is aligning the served .mjs with the bundled entry) at an expected ~1 s, and threaded WASM activates automatically (probe is in the worker) once the renderer becomes crossOriginIsolated. First-use requires one network fetch — accepted explicitly over installer bloat.
