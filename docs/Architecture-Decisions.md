...
**Amendment (v0.8).** **The WebGPU backend is now the default** wherever `navigator.gpu` yields a device: it is measurably faster (100k shapes at 60fps against a Canvas2D budget aimed at typical documents) and its parity fixtures have grown to 14/14, so leaving the faster renderer switched off was costing everyone the difference to buy soak time from nobody. Nothing about the fallback changes — no device, or a device that fails to initialise, still lands on Canvas2D silently. What did change is that the editor now distinguishes the **preference** (`gpuRender`, remembered) from **what is actually drawing** (`gpuActive`, reported by the canvas), because a tick that survives a failed device is a claim about your renderer that is not true (F-30).
...
- **Installing is off by DEFAULT rather than absent** (revised within v0.8, at the maintainer's request). The argument above is about verifying bytes and it has not improved — nothing is signed — so what changed is who decides, and when: a download starts because someone pressed *Download and install*, or because they ticked *install automatically* and can untick it, and the words "not code signed yet" sit beside the button rather than in a document. `INSTALL_UPDATES` as a constant is gone; the switch is a setting, off until asked for. macOS is not offered it at all — Squirrel.Mac applies only updates signed by the running app's team, so there the button opens the release page and `canInstall` states that as a fact instead of a platform check copied into the UI.
...
Also decided here: a shape node's children are a boolean's **operands** and are dropped, because the flattened result already contains them — drawing both is drawing the answer twice.

**Correction (v0.8, F-32).** Four more, from a 4,825-node file. Each is a case of reading our own vocabulary or our own output where the file had an answer.

- **A Figma page becomes a Polyform page.** The debt above is paid: `addNode` already accepted a page id as its parent, so the missing piece was only a `page-add` op recorded in the same commit as the nodes. Pages are no longer shifted apart, every node keeps the coordinates its own page gave it, and an untouched document's starter page is renamed and reused rather than left over.
- **Their enum, in their spelling.** The winding rule is `NONZERO | ODD`; comparing against `EVENODD` — our name for the rule — read every even-odd path as nonzero. The value was never unknowable: the schema is *inside the file*. Any constant compared against a foreign format is now quoted from that format's own definition, and the fixture that covers it carries their value, not ours.
