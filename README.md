...
- **WebGPU rendering (beta)** — View → GPU Rendering switches the scene onto a lyon-tessellated, batched WebGPU pipeline: **100,000 shapes pan at 60fps** (0.18 ms CPU per frame, one draw call — verified by the in-app harness), with **11/11 pixel-parity fixtures** against the Canvas2D reference — drop/inner shadows, layer & background blur, all 16 blend modes through a bake-time effects compositor (ADR-017), and shaped text from a GPU glyph atlas (ADR-018). Canvas2D remains the default renderer.
...
| `npm run test:mcp`  | Agent-connectivity probe: 26 checks — a real MCP client vs the built app, driving the consent panel |
