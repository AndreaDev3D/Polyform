...
- **Agent connectivity (in progress)** — an MCP server inside the app lets an AI agent attach to the *running* editor: read the open document and selection, and watch your edits stream in through the journal. Off by default, loopback-only, bearer-token protected ([ADR-021](docs/Architecture-Decisions.md)). Journaled agent *edits* and the headless CLI are next.
